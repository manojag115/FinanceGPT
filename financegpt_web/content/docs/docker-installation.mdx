...
description: Setting up FinanceGPT using Docker
---
...
This guide explains how to run FinanceGPT using Docker, with options ranging from quick single-command deployment to full production setups.

## Quick Start with Docker 🐳

Get FinanceGPT running in seconds with a single command:

<Callout type="info">
The all-in-one Docker image bundles PostgreSQL (with pgvector), Redis, and all FinanceGPT services. Perfect for quick evaluation and development.
</Callout>

<Callout type="warn">
Make sure to include the `-v financegpt-data:/data` in your Docker command. This ensures your database and files are properly persisted.
</Callout>
...
curl -o docker-compose.yml https://raw.githubusercontent.com/MODSetter/FinanceGPT/main/docker-compose.quickstart.yml
...
```bash
# View logs
docker logs -f financegpt

# Stop FinanceGPT
docker stop financegpt

# Start FinanceGPT
docker start financegpt

# Remove container (data preserved in volume)
docker rm financegpt

# Remove container AND data
docker rm financegpt && docker volume rm financegpt-data
```
...
1. **Configure Environment Variables**
    ...
    **Linux/macOS:**
    ...
    cp financegpt_web/.env.example financegpt_web/.env
    ...
    **Windows (Command Prompt):**
    ...
    ```cmd
    copy financegpt_backend\.env.example financegpt_backend\.env
    copy financegpt_web\.env.example financegpt_web\.env
    ...
    **Windows (PowerShell):**
    ...
    ```powershell
    Copy-Item -Path financegpt_backend\.env.example -Destination financegpt_backend\.env
    Copy-Item -Path financegpt_web\.env.example -Destination financegpt_web\.env
...
| POSTGRES_DB                | PostgreSQL database name                                                    | financegpt           |
| PGADMIN_DEFAULT_EMAIL      | Email for pgAdmin login                                                     | admin@financegpt.com |
| PGADMIN_DEFAULT_PASSWORD   | Password for pgAdmin login                                                  | financegpt           |
...
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| DATABASE_URL               | PostgreSQL connection string (e.g., `postgresql+asyncpg://postgres:postgres@localhost:5432/financegpt`)                                                                                    |
...
| LANGSMITH_PROJECT | LangSmith project name (e.g., `financegpt`) |
...
2. Login with the credentials from your `.env` file (default: admin@financegpt.com / financegpt)
3. Right-click "Servers" > "Create" > "Server"
4. In the "General" tab, name your connection (e.g., "FinanceGPT DB")
5. In the "Connection" tab:
   - Host: `db`
   - Port: `5432`
   - Maintenance database: `financegpt`
...
Once your installation is complete, you can start using FinanceGPT! Navigate to the frontend URL and log in using your Google account.
