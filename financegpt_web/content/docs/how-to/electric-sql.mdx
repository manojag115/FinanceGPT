...
The simplest way to run FinanceGPT with Electric SQL is using the all-in-one Docker image. This bundles everything into a single container:
...
Access FinanceGPT at `http://localhost:3000`. Electric SQL is automatically configured and running on port 5133.
...
electric:
  ...
  environment:
    - DATABASE_URL=${ELECTRIC_DATABASE_URL:-postgresql://${ELECTRIC_DB_USER:-electric}:${ELECTRIC_DB_PASSWORD:-electric_password}@${POSTGRES_HOST:-db}:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-financegpt}?sslmode=disable}
...
Ensure your environment files are configured. If you haven't set up FinanceGPT yet, follow the [Manual Installation Guide](/docs/manual-installation) first.
...
**Frontend `.env` (`financegpt_web/.env`):**
...
2. Delete databases starting with `financegpt-`
