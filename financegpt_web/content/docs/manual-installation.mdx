...
description: Setting up FinanceGPT manually for customized deployments (Preferred)
---

# Manual Installation (Preferred)

This guide provides step-by-step instructions for setting up FinanceGPT without Docker. This approach gives you more control over the installation process and allows for customization of the environment.
...
The backend is the core of FinanceGPT. Follow these steps to set it up:
...
```bash
# Make sure you're in the financegpt_backend directory
cd financegpt_backend
...
The FinanceGPT browser extension allows you to save any webpage, including those protected behind authentication.
...
```bash
cd financegpt_browser_extension
cp .env.example .env
```
...
| ENV VARIABLE              | DESCRIPTION                                           |
| ------------------------- | ----------------------------------------------------- |
| PLASMO_PUBLIC_BACKEND_URL | FinanceGPT Backend URL (e.g., `http://127.0.0.1:8000`) |
...
Load the extension in your browser's developer mode and configure it with your FinanceGPT API key.
...
Now that you have FinanceGPT running locally, you can explore its features:
